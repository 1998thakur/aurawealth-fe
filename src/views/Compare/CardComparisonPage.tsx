...
export default function CardComparisonPage() {
  ...
  useEffect(() => {
    ...
    const base = pathname ?? '/compare';
    if (ids.length > 0) {
      router.replace(`${base}?cards=${ids.join(',')}`, { scroll: false } as Parameters<typeof router.replace>[1]);
    } else {
      router.replace(base, { scroll: false } as Parameters<typeof router.replace>[1]);
    }
  }, [cardIds, pathname, router]);
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {validCards.length >= 2 ? (
          ...
          <div className="text-center py-12 bg-surface-container-low rounded-2xl border border-outline-variant">
            ...
            <Link href="/cards" className="btn-outlined text-sm">
              Browse Card Catalog
            </Link>
          </div>
        )}
